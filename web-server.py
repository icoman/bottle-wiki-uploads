...
from modeldb import Base, WikiPages, WikiVersions, Users, Files, addUsers, addPages
...
DSN = os.getenv('DSN','sqlite:///wiki-uploads.db')
...
def callback(db):
    print "clean"
    #get metadata and delete all tables
    metadata = MetaData(db.bind)
    metadata.reflect()
    metadata.drop_all()
    metadata.create_all()
    return 'DB Clean - <a href="/">Home</a>'
'''
...
def callback(db, id):
    obfile = db.query(Files).filter(Files.Id == id).one()
    #saved_file = "%s/%d/%s" % (uploads_folder, obfile.owner.id, obfile.Filename)
    saved_file = "%d/%s" % (obfile.owner.id, obfile.Filename)
    bottle.response.content_type = obfile.ContentType
    #return bottle.static_file(saved_file, root='')
    return bottle.static_file(saved_file, root=uploads_folder)

@app.route('/upload/<path:path>',method=['POST'])
...
def callback(db, path):
    ...
    if upload:
        ...
        #print name, "--",ext
        content_type = upload.content_type.replace('"','')
        try: os.mkdir(uploads_folder + os.sep + str(page.id))
        except: pass
        saved_file = "%s/%d/%s" % (uploads_folder, page.id, upload.filename)
        print "Upload",upload.filename,":",
        BUFFSIZE = 1024*1024
        f = open(saved_file, "wb")
        while True:
            data = upload.file.read(BUFFSIZE)
            if not data:
                break
            f.write(data)
            print "#", #display upload progress, a # for each megabyte
        print "-end"
        size = f.tell()
        f.close()
        obfile = Files(upload.filename, size, content_type, page.id)
        db.add(obfile)
...
def callback(db, path):
    ...
    if 0 == q.count():
        ...
        return dict(title=title, user=user, body=body, path=path, pageuser=pageuser, created=created, allversions=allversions, d_users=d_users, files=files)
